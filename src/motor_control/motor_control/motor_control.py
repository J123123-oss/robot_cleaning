...
BOUNDARY_SLOW_PERSIST_FRAMES = 5  # 单传感器慢速通道：持续N帧(0.5s@10Hz)触发
...
class MotorControlNode(Node):
    def __init__(self, node_name='motor_control_node'):
        ...
        # Web 遥控器速度缓存（由 /web_rc/motor_speed 回调更新）
        self.web_rc_left_speed = 0.0
        self.web_rc_right_speed = 0.0
        self.web_rc_brush_speed = 0.0
        self.web_rc_last_update = 0.0
        self.web_rc_timeout = 1.0  # 超时阈值（秒）

        # Web 遥控器独占控制标志（由 /web_rc/control_active 回调更新）
        ...
        self.web_rc_sub = self.create_subscription(
            Vector3,
            "/web_rc/motor_speed",
            self.web_rc_speed_callback,
            10
        )
        self.web_rc_control_sub = self.create_subscription(
            Bool,
            "/web_rc/control_active",
            self.web_rc_control_callback,
            10
        )
    ...
    def build_error_code(self) -> int:
        ...
        error |= self.rtk_error_code & (
            ERROR_RTK_NOT_FIXED
            | ERROR_RTK_TIMEOUT
            | ERROR_HEADING_STABILITY_TIMEOUT
            | ERROR_CALIB_TIMEOUT
        )
    ...
    def timer_callback(self):
        ...
        # ═══════════════════════════════════════════════════════════
        # Web 遥控器 MANUAL 模式：最高优先级，独占控制
        # 收到 [Web] MANUAL 后激活，[Web] AUTO 后释放
        # 此模式下忽略所有其他控制来源（键盘/MQTT/RTK/rc_control）
        # ═══════════════════════════════════════════════════════════
        if self.web_control_active:
            self.current_control_mode = "WEB_REMOTE"

            # 防跌落边界保护：传感器触发时强制停车（安全优先）
            if self.current_status in self.blocked_directions:
                self.set_motors_speed(0.0, 0.0)
                self.set_brush_speed(0.0)
            # 进出仓流程暂停
            elif self.is_in_bin_process:
                self.set_motors_speed(0.0, 0.0)
                self.set_brush_speed(0.0)
            else:
                # 应用 Web 遥控器速度（带超时保护）
                if time.time() - self.web_rc_last_update < self.web_rc_timeout:
                    self.set_motors_speed(self.web_rc_left_speed, self.web_rc_right_speed)
                else:
                    if self.current_left_speed != 0.0 or self.current_right_speed != 0.0:
                        self.get_logger().warn("[WebRC] 速度数据超时，自动停车")
                    self.set_motors_speed(0.0, 0.0)
                self.set_brush_speed(self.web_rc_brush_speed)

            # 发布控制模式
            mode_msg = String()
            mode_msg.data = "WEB_REMOTE"
        ...
        if self.current_control_mode == "NORMAL":
            # Web 遥控器控制（rc_control=True 时优先使用串口解析的连续速度）
            if self.rc_control:
                if time.time() - self.web_rc_last_update < self.web_rc_timeout:
                    self.set_motors_speed(self.web_rc_left_speed, self.web_rc_right_speed)
                else:
                    # 超时保护：超过阈值未收到遥控器数据则停车
                    self.set_motors_speed(0.0, 0.0)
                self.set_brush_speed(self.web_rc_brush_speed)
            # 防跌落：传感器触发时仅禁止对应方向，其他方向正常可用
            elif self.current_status in self.blocked_directions:
                self.set_motors_speed(0.0, 0.0)
                self.set_brush_speed(0.0)
    ...
    def keyboard_callback(self, msg: String) -> None:
        ...
        # Web 遥控器 MANUAL 模式：独占控制，屏蔽所有键盘指令
        if self.web_control_active:
            self.get_logger().warn(
                f"[WebRC] MANUAL模式独占控制中，忽略键盘指令: {key} "
                f"(发送 [Web] AUTO 退出MANUAL模式)"
            )
            return
    ...
    def web_rc_speed_callback(self, msg: Vector3):
        """订阅 Web 遥控器解析节点发布的速度指令"""
        self.web_rc_left_speed = float(msg.x)
        self.web_rc_right_speed = float(msg.y)
        self.web_rc_brush_speed = float(msg.z)
        self.web_rc_last_update = time.time()

    def web_rc_control_callback(self, msg: Bool):
        """订阅 Web 遥控器控制权状态（MANUAL=独占 / AUTO=释放）"""
        was_active = self.web_control_active
        self.web_control_active = msg.data
        if was_active != self.web_control_active:
            if self.web_control_active:
                self.get_logger().info("[WebRC] 🟢 收到 MANUAL，Web 遥控器独占控制已激活")
            else:
                self.get_logger().info("[WebRC] 🔴 收到 AUTO，Web 遥控器控制权已释放")
                # 释放控制权后立即停车，等待后续指令
                self.set_motors_speed(0.0, 0.0)
                self.set_brush_speed(0.0)
    ...
    def io_data_callback(self, msg: UInt8):
        ...
        changed = False
        # 已确认但当前帧不再触发的传感器 → 立即移除（去抖只防误加，不防移除）
    ...
    def _update_blocked_directions(self):
        """
        根据 confirmed_sensors 计算禁止的行驶方向。
        - 前部传感器(mid)触发 → 禁止 FORWARD
        - 后部传感器(back)触发 → 禁止 BACKWARD
        - 左侧传感器触发 → 禁止 LEFT
        - 右侧传感器触发 → 禁止 RIGHT
        被禁止的方向在 timer_callback 中会被忽略（等同HOLD）。
        """
        ...
        if old != new:
            self.get_logger().info(
                f"[Motor] 禁止方向更新：{sorted(old) if old else '无'} → {sorted(new) if new else '无'}，"
                f"confirmed_sensors={sorted(self.confirmed_sensors)}")
        if self.current_status in self.blocked_directions:
            self.get_logger().warn(
                f"[Motor] 当前状态{self.current_status}已被传感器禁止，立即停车")
            self.set_motors_speed(0.0, 0.0)
            self.set_brush_speed(0.0)
    ...
    def status_callback(self, msg):
        ...
        try:
            ...
            # Web 遥控器 MANUAL 模式：仅允许 GET_STATUS 和 DISABLE_CAMERA_REMOTE
            if self.web_control_active and command not in ("GET_STATUS", "DISABLE_CAMERA_REMOTE"):
                self.get_logger().warn(
                    f"[WebRC] MANUAL模式独占控制中，忽略MQTT指令: {command} "
                    f"(发送 [Web] AUTO 或云端 DISABLE_CAMERA_REMOTE 退出MANUAL模式)"
                )
                return
            ...
            if command == "GET_STATUS":
                ...
                self.publish_state()
            elif command == "DISABLE_CAMERA_REMOTE":
                # 云端强制释放摄像头遥控器独占控制权
                # 无论当前处于什么模式，一律释放并切到安全状态
                was_web_active = self.web_control_active
                was_rc = self.rc_control
                was_auto = (self.current_control_mode == "AUTO_CLEANING")
                ...
                if was_rc:
                    parts.append("RC遥控器")
                if was_auto:
                    parts.append("AUTO_CLEANING")
                self.get_logger().info(
                    f"[ROSNode] DISABLE_CAMERA_REMOTE 云端强制释放控制权完成，"
                    f"已释放: {', '.join(parts) if parts else '无活跃控制源'}，"
                    f"当前状态: HOLD + NORMAL")
                self.publish_state()
            ...
            # Web 遥控器 MANUAL 模式：仅允许 GET_STATUS 和 DISABLE_CAMERA_REMOTE
            if self.web_control_active and raw_command not in ("GET_STATUS", "DISABLE_CAMERA_REMOTE"):
                self.get_logger().warn(
                    f"[WebRC] MANUAL模式独占控制中，忽略MQTT指令: {raw_command} "
                    f"(发送 [Web] AUTO 或云端 DISABLE_CAMERA_REMOTE 退出MANUAL模式)"
                )
                return

            # AUTO_CLEANING模式下只允许HOLD和RC_ENABLE/RC_DISABLE/DISABLE_CAMERA_REMOTE指令
            if is_rtk_nav_mode and raw_command not in ["HOLD", "RC_ENABLE", "RC_DISABLE", "DISABLE_CAMERA_REMOTE"]:
                self.get_logger().warn(f"[ROSNode] AUTO_CLEANING模式下拒绝指令: {raw_command}，仅允许HOLD/RC_ENABLE/RC_DISABLE/DISABLE_CAMERA_REMOTE")
                return
            if raw_command == "DISABLE_CAMERA_REMOTE":
                # 字符串格式的 DISABLE_CAMERA_REMOTE：复用 JSON 路径的同一逻辑
                # 构造临时 dict 传入已有处理分支会更好，但这里直接执行释放
                was_web_active = self.web_control_active
                was_rc = self.rc_control
                was_auto = (self.current_control_mode == "AUTO_CLEANING")
                ...
                if was_auto:
                    parts.append("AUTO_CLEANING")
                self.get_logger().info(
                    f"[ROSNode] DISABLE_CAMERA_REMOTE(string) 云端强制释放控制权完成，"
                    f"已释放: {', '.join(parts) if parts else '无活跃控制源'}，"
                    f"当前状态: HOLD + NORMAL")
            elif raw_command in STATE_DICT:
    ...
    def fail_loading_process(self, reason: str) -> None:
        """进仓前置/对准失败：停车、上报超时故障并退出进仓流程。"""
    ...
    def handle_loading_step(self):
        ...
        if self.loading_phase == "LOADING_NAV_TO_GPS":
            ...
            if self.current_lon == 0.0 and self.current_lat == 0.0:
                self.fail_loading_process("[LOADING] GPS坐标无效，停车并上报进仓故障")
                return
            ...
            if current_time - self.loading_nav_start_time > self.loading_nav_timeout:
                self.fail_loading_process(
                    ...
                    f"当前距离{gps_dist:.1f}m，停车并上报进仓故障")
                return
        ...
        if self.loading_phase == "LOADING_DIRECT_BACKWARD":
            ...
            if elapsed_time < self.loading_direct_timeout:
                ...
                # ── 航向差速纠偏：后退时根据偏角修正左右轮差速 ──
                if self.imu_yaw_deg is not None:
                    heading_err = self.get_heading_error(self.loading_turn_target_deg)
                    if abs(heading_err) < NAV_DRIVE_DEADZONE:
                        correction = 0.0
                    else:
                ...
                if current_time - self.last_direct_loading_log_time >= 5.0:
                    self.get_logger().warn(
                        f"[LOADING] 激光错误进仓兜底中，已后退{elapsed_time:.1f}s/"
                        f"{self.loading_direct_timeout:.0f}s, "
                        f"航向{self.imu_yaw_deg:.1f}°, "
                        f"目标{self.loading_turn_target_deg:.1f}°, "
                        f"偏角{heading_err:+.1f}°, 纠偏{correction:+.2f}"
                    )
    ...
    def _handle_loading_step_inner(self):
        ...
        if self.bin_process_paused:
            return
        # 当 loading_skip_dock_check 标志位为 True 时，跳过 dock 传感器限位检测
        if not self.loading_skip_dock_check:
            ...
            self._dock_sensor_blocked_logged_loading = False
        # dock中归位 或 跳过dock检测标志位为True → 允许进入进仓流程
        if (self.dock_sensors & 0x02) or self.loading_skip_dock_check:
            ...
            try:
                ...
                if self.loading_phase == "LOADING_TURN":
                    ...
                    right_speed = turn_speed + correction
                    self.set_motors_speed(left_speed, right_speed)
                    # 诊断日志：每1s打印一次IMU更新时间差，用于排查角度更新频率问题
                    if not hasattr(self, '_last_turn_diag_time'):
                    ...
                    if current_time - self._last_turn_diag_time >= 1.0:
                        ...
                        self.get_logger().info(
                            ...
                            f"turn={turn_speed:.2f}, corr={correction:.2f}, "
                            f"L={left_speed:.2f}, R={right_speed:.2f}, "
                            f"motor_L={actual_left:.1f}, motor_R={actual_right:.1f}, "
                            f"imu_age={imu_age:.2f}s")
                        self._last_turn_diag_time = current_time
