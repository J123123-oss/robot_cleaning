...
当一个区域包含多个相接或错位 polygon 时，规划器会先生成所有扫描段，再在安全连接图上优化扫描顺序：优先压低最长连接距离，其次压低连接总距离，并把下一个桥架端点纳入最终出口选择。这样带有加长块或窄通道的区域不会机械地把支路留到最后，再横向返回下一个桥架。最多 16 条扫描段的多 polygon 区域使用精确状态搜索；更大的区域使用有界 beam search 控制规划时间，同时保留出口候选。`--max-connector` 同时约束优化阶段和最终输出阶段。
