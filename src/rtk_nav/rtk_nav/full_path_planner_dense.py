...
def interpolate_segment(a, b, spacing):
    """在两个UTM点之间按spacing插值，返回包含起点和终点的点列表"""
    ax, ay = a
    bx, by = b
    dx = bx - ax
    dy = by - ay
...
def generate_dense_from_utm(path_utm, spacing):
    ...
    for i in range(len(path_utm) - 1):
        ...
        # 跳过重复的端点（连接处的点）
        if dense and seg_pts:
            if math.hypot(dense[-1][0] - seg_pts[0][0], dense[-1][1] - seg_pts[0][1]) < 0.1:
                seg_pts = seg_pts[1:]
...
class MultiAreaCleaningPathPlanner(Node):
    def __init__(self):
        ...
        self.declare_parameter('config_file', '/home/ubuntu/robot_cleaning/src/rtk_nav/rtk_nav/config/004_north24-18.yaml')
    ...
    def _plot_multi_area_path(self, merged_path_utm, all_orig_corners, all_inner_corners, utm_zone, save_dir, timestamp, all_calib_points_utm, area_names, dense_utm=None, area_index_ranges=None):
        ...
        # 标注每个区域的名称
        # if area_index_ranges is not None:
        #     for start_idx, end_idx, area_name in area_index_ranges:
        #         if start_idx < len(merged_path_utm):
        #             center_e = merged_path_utm[start_idx][0]
        #             center_n = merged_path_utm[start_idx][1]
        #             ax.annotate(f'#{area_name}', (center_e, center_n), 
