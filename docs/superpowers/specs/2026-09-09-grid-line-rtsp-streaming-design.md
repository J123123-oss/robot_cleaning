...
H.264 RTSP stream to an existing media server. VLC or another RTSP-capable
backend player pulls the corresponding RTSP playback URL directly.

## Confirmed Requirements

- The backend player pulls the final RTSP playback URL; the robot does not
  push to the playback URL shown by the backend UI.
...
- The backend/media-server deployment is outside this repository and must
  provide an RTSP publish endpoint plus an RTSP playback endpoint.
...
- Convert the BGR image explicitly to RGB raw frames for an FFmpeg subprocess.
...
- Document the external FFmpeg dependency and the relationship between the
  RTSP publish URL and the RTSP playback URL.
...
2. The worker converts the newest message to `bgr8`, swaps it to RGB before
   starting FFmpeg, and writes raw RGB frames after the first valid frame
   establishes the input dimensions.
...
1. The FFmpeg command uses explicit BGR-to-RGB conversion followed by raw RGB
   input, low-latency H.264, RTSP TCP output, the configured dimensions/rate/
   bitrate, and no `shell=True`.
...
receives the RTSP stream and that the backend can pull its RTSP URL. Those
checks require deployment-specific server addresses and are not part of the
repository unit test suite.

## Operational Contract

The backend configuration must use the RTSP playback address generated by the
