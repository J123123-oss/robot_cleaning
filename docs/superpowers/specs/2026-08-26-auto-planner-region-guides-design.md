...
- YAML 未提供根级 `guides` 时，转换器从生成后的全部区域边界中选择实际最长边，写入
  一个两点根级 guide，作为地图级元数据和旧版单区域地图的默认方向。
...
- 生成的 v2 map 在 `regions[].guide` 中保存区域方向；根级 `guides` 仍保存实际轨迹
  点，不改为方向字符串。
- 不配置区域 guide 时，区域从自身全部多边形边界中选择最长实际边作为清扫方向；
  这条规则独立于根级 guide，因此不同子区域可以自然保持不同方向。

## Architecture

`Region` 增加可选的 `guide` 字段。v2 解析器验证方向值，并在 GeoJSON 边界属性中
暴露它。规划入口为每个区域解析独立轴角：配置了 guide 时映射为水平 `0` 弧度或
垂直 `pi / 2` 弧度；未配置时从该区域边界选择最长边。区域覆盖扫描、区域内连接
和进出区域的起止点使用该区域轴角。
...
- v2 地图不含 `regions[].guide` 时，每个区域使用自身最长边；没有区域结构的旧版
  单区域地图继续使用根级 guide 或最长边回退。
...
轴角映射、区域未配置时按本区域最长边和非法值报错；E13 两个 YAML 多边形共享逻辑
区域 guide；转换后的 full map 经过 `load_map` 和 `plan_route` 后各区域 coverage 使用
各自方向，且桥架顺序和现有 E12-E14 输出契约不变。
