...
## 2026-08-12 首次清扫前增加绝对出仓航向区间校验

### 97. 航向稳定但方向错误时禁止直接启动清扫

- **问题**：出仓完成后，航向稳定性窗口只能判断 IMU 是否继续漂移，不能判断车头是否朝向现场约定的清扫方向。日志中 `1786495671.055616283` 已报告 5 秒波动 `0.45°`、30 秒收敛波动 `2.00°` 并通过 AUTO 门控，但随后 `1786495671.084397243` 的 `IMU=152.6°` 仍被放行到 `IDLE`，继而启动初始移动；该方向与现场出仓清扫方向相反，后续还会继续漂移。
- **修复**：在首次出仓后进入清扫的 AUTO 航向门控中增加绝对航向范围 `INITIAL_HEADING_MIN <= imu_yaw <= INITIAL_HEADING_MAX`，当前范围为 `-100° <= imu_yaw <= -80°`。稳定窗口和收敛窗口均通过、但航向超出该范围时，记录“航向稳定但方向错误”，清空稳定样本并重置门控计时，保持零速度重新等待下一组稳定航向。
- **适用范围**：绝对区间只用于首次出仓后开始清扫；运行中 RTK Fixed 恢复或 WAYPOINT_MOVE 姿态突变恢复，仍复用航向稳定性校验，不套用固定的出仓方向区间。
- **通过条件**：必须同时满足 RTK 双 Fixed、GGA 有效、5 秒波动不超过 `1°`、30 秒收敛波动不超过 `1°`，以及 IMU 航向位于 `[-100°, -80°]`（含边界）；通过后才允许 AUTO 恢复并启动初始移动。
- **验证**：`test_heading_recovery_gate_contract.py` 增加绝对区间常量、首次清扫门控启用条件、稳定后越界清空窗口及运行中不重复校验的契约断言；已通过 Python 语法和文档差异检查，未进行 ROS/实车验证。
- **影响**：`src/rtk_nav/rtk_nav/rtk_nav.py` 的 AUTO 初始航向门控；稳定但为 `152.6°` 等错误方向的航向不再直接进入清扫。
